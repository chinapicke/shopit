    ...
    <>
      ...
      <div className='topPicks grid grid-cols-2 py-4 px-6 md:grid-cols-4 lg:grid-cols-4'>
        {filterById.map((item, index) => {
          return (
            <div className='topProductCard border mx-2' key={item.id}>
              ...
              <Link to={`/product/${item.id}`} name={item.brand}>
                <img className='topPickImage ml-5 mb-2 md:ml-12' src={item.api_featured_image} alt={item.brand + item.product_type}></img>
                {/* To display the brand name with as sentence case */}
                <div className='topPickText px-2 '>
                ...
                <p className='topPickNumber'>£{Number(item.price).toFixed(2)
                }</p>
                </div>
              </Link>
