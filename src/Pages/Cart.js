            ...
            <div className='addCardCart my-4' key={index}>
              <div className='innerAddcardCartPg grid grid-cols-2 '>
                <div className='flex items-center'>
                  <img className='w-36 h-40 py-2 px-2 md:w-44 md:h-44 lg:w-48 lg:h-48' src={item.api_featured_image} alt={item.brand + item.product_type}></img>
                ...
                <div className="flex flex-col justify-center">
                  ...
                  <h2 className="cartBrand lg:text-lg">{item?.brand ? item.brand.charAt(0).toUpperCase() + item.brand.slice(1).toLowerCase() : item.brand} {item?.name ? item.name.charAt(0).toUpperCase() + item.name.slice(1).toLowerCase() : item.name}</h2>
                  ...
                  <div className="flex flex-row justify-between mx-3 my-4 md:mx-12">
                    <button className='cartPlus' onClick={() => dispatch({ type: "PLUS", payload: item })}>
                      +
                    </button>
                    <p className='cartQuantity bg-white px-3 py-1 border-2'>{item.quantity}</p>
                  ...
                  <h4 className='totalCartItemPrice my-2 text-center md:hidden'> £{(item.price * item.quantity).toFixed(2)}</h4>
                </div>
                <div className="flex items-center justify-center">
                  <h4 className='totalCartPriceLargeScreen my-2 text-center sm:hidden md:block '> £{(item.price * item.quantity).toFixed(2)}</h4>
                </div>
                <div className='cartDeleteLargeScreen flex justify-center items-center '>
        ...
        <>
          <h1> Subtotal:<span>£{total.toFixed(2)}</span></h1>
          {
            total >= 30.00 ?
              ...
              <h2>Delivery: <span>£2.99</span> </h2>
          }
          {
            total >= 30.00 ?
              <h2>Total: <span>£{total.toFixed(2)}</span></h2> :
              <h2>Delivery: <span>£{(parseInt(total.toFixed(2)) + 2.99).toFixed(2)}</span> </h2>
          }
        </>
      )}
    </div >
